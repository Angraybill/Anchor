...
The executable suite covers fixture candidate filtering, driver-then-rider mutual acceptance, final-seat contention, driver cancellation into Rescue mode, rider cancellation without Rescue mode, stale acceptance rejection, server-derived offer ownership, input validation, candidate authorization, pickup-reveal privacy, private UUID-only Realtime topics, and rejection of service-role keys in browser configuration. Run it with `npm test`.
